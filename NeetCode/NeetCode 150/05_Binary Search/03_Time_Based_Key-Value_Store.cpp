...
public:
    ...
    string get(string key, int timestamp) {
        ...
        // If we reach here means no value of timestamp is a exact match,
        // so we need to return just smaller value of the time stamp
        // since e and s are crossed so e will be at the index just smaller that
        // the target
        if (e >= 0) {
            return m[key][e].second;
        }
        // if e is negative return null string
        return "";
    }
};
